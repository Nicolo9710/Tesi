...
class FedXgbBagging(FedAvg):
    ...
    def evaluate(
        ...
        # eval_res = self.evaluate_function(server_round, model, {})
    ...
    bst_curr_org: bytes,) -> bytes:
        """Conduct bagging aggregation for given trees."""
        if not bst_prev_org:
            return bst_curr_org

        # Get the tree numbers
        tree_num_prev, _ = FedXgbBagging._get_tree_nums(bst_prev_org)
        _, paral_tree_num_curr = FedXgbBagging._get_tree_nums(bst_curr_org)
    ...
    def aggregate_fit(
        ...
        for _, fit_res in results:
            ...
            for bst in update:
                global_model = FedXgbBagging.aggregate(global_model, bst)
