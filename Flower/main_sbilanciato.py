...
#df = df = dd.read_csv('C:/Users/lukyl/OneDrive/Desktop/dataset_tesi_CIC_IDS2017/NF_ToN_IoT_V2/output_file4.csv')
...
for i in tqdm(range(n_partitions)):
    ...
    # X = p.drop(columns=['IPV4_SRC_ADDR', 'IPV4_DST_ADDR', 'Label', 'Attack', 'Attack_label'])
    ...
    # # assegno circa 338k righe ad ogni client, mantenedo la distribuzione dati originale (riferita alla partizione di quell'indirizzo ip)
    # x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size=n, shuffle = True)

    # # divido in test, train e validation per ogni client
    ...
    file_path_test = f"C:/Users/lukyl/OneDrive/Desktop/dataset_tesi_CIC_IDS2017/NF_ToN_IoT_V2/Dmatrix_sbilanciato/test_matrix{i}.bin"
